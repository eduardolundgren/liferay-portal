AUI.add(
	...
	function(A) {
		...
		var CSS_DIAGRAM_BUILDER_DROP_CONTAINER = '.diagram-builder-drop-container';
		...
		var STR_DDM_PAGE = 'ddm-page';

		var STR_FIELDS = 'fields';
		...
		var LiferayFormBuilder = A.Component.create(
			{
				...
				prototype: {
					initializer: function() {
						...
						instance.after(
							...
							function(event) {
								...
								if (instance._getPageBreakCount() > 0) {
					...
					getXSD: function() {
						...
						var pageBreak = (instance._getPageBreakCount() > 0);

						if (pageBreak) {
							instance._createNestedPageStructure();
						}

						instance.get(STR_FIELDS).each(
					...
					_createNestedPageStructure: function() {
						var instance = this;

						var container = A.one(CSS_DIAGRAM_BUILDER_DROP_CONTAINER);
					...
					_flattenPageFields: function() {
						...
						var fields = instance.get(STR_FIELDS);

						var size = fields.size();

						fields.each(
							function(item1, index1, collection1) {
								item1.get(STR_FIELDS).each(
									function(item2, index2, collection2) {
										pageFields.push(item2);
									}
								);

								item1.set(STR_FIELDS, []);

								if (!((index1 === (size - 1)) && (item1.name === STR_DDM_PAGE))) {
									pageFields.push(item1);
								}
							}
						);

						instance.set(STR_FIELDS, pageFields);
					},

					_getPageBreakCount: function() {
						var instance = this;

						return A.all(CSS_DIAGRAM_BUILDER_DROP_CONTAINER + STR_SPACE + CSS_PAGE_BREAK).size();
					},
					...
					_setFieldsNestedStructure: function() {
						...
						var pages = instance.get(STR_FIELDS).filter(
							function(item, index, collection) {
								if (item.name == STR_DDM_PAGE) {
									item.set(STR_FIELDS, fields);
						...
						if (fields.length) {
							...
							var page = instance.createField(new A.FormBuilder.types[STR_DDM_PAGE](config));

							page.set(STR_FIELDS, fields);

							pages.add(page);
						}

						instance.set(STR_FIELDS, pages);
					},
					...
					_setFieldsSortableListConfig: function(val) {
						...
						config.dropCondition = function(event) {
							...
							if (dragField && dragField.get('type') == STR_DDM_PAGE || dropField.get('type') == STR_DDM_PAGE) {
								return false;
							}
					...
					_syncFieldsLocaleUI: function(locale, fields) {
						...
						fields = fields || instance.get(STR_FIELDS);
						...
						fields.each(
							function(field, index, fields) {
								...
								instance._syncFieldsLocaleUI(locale, field.get(STR_FIELDS));
							}
						);
					},
					...
					_updateFieldsLocalizationMap: function(locale, fields) {
						...
						fields.each(
							function(item, index, collection) {
								...
								instance._updateFieldsLocalizationMap(locale, item.get(STR_FIELDS));
							}
						);
					}
				},
